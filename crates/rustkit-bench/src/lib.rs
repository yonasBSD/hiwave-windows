...
impl BenchmarkSuite {
    ...
    pub fn print_summary(&self) {
        ...
        println!("{}", "=".repeat(80));
        println!("{:40} {:>12} {:>12} {:>12}", "Name", "Mean", "StdDev", "Throughput");
        println!("{}", "-".repeat(80));
    ...
    pub fn save_json(&self, path: &str) -> Result<(), BenchError> {
        let json = serde_json::to_string_pretty(self)
            .map_err(|e| BenchError::Failed(e.to_string()))?;
...
impl Benchmark {
    ...
    where
        F: FnMut(),
    {
        debug!(name, warmup = self.warmup, iterations = self.iterations, "Running benchmark");
    ...
    fn bench_layout_nested(&self) -> BenchmarkResult {
        ...
        self.run("layout/nested (10 boxes)", || {
            ...
            // Add nested children
            for _ in 0..10 {
                root.children.push(LayoutBox::new(BoxType::Block, style.clone()));
            }
...
fn generate_css(n: usize) -> String {
    ...
    for i in 0..n {
        css.push_str(&format!(".class{} {{ margin: {}px; padding: {}px; }}\n", i, i, i * 2));
    }
    css
}
